...
prepare::
	...
	if [[ "${OS}" != "Darwin" ]]; then \
		sed -i 's,github.com/MaterializeInc/pulumi-frontegg,${PROJECT},g' provider/go.mod; \
	...
	if [[ "${OS}" == "Darwin" ]]; then \
		sed -i '' 's,github.com/MaterializeInc/pulumi-frontegg,${PROJECT},g' provider/go.mod; \
		find ./ ! -path './.git/*' -type f -exec sed -i '' 's/[x]yz/${NAME}/g' {} \; &> /dev/null; \
		find ./ ! -path './.git/*' -type f -exec  sed -i '' 's,github.com/MaterializeInc/pulumi-frontegg,${PROJECT},g' {} \; &> /dev/null; \
	fi
...
build_sdks:: install_plugins provider build_python # build all the sdks
